...
namespace {
    ...
    public:
        ...
        void test_running_cross_covariance_sparse() {
            ...
            running_cross_covariance<matrix<double> > rcc = rcc1 + rcc2;
            DLIB_TEST_MSG(max(abs(rcc.covariance_xy() - cov)) < 1e-14,
                          max(abs(rcc.covariance_xy() - cov)));
            DLIB_TEST(max(abs(rcc.mean_x() - xm)) < 1e-14);
            DLIB_TEST(max(abs(rcc.mean_y() - ym)) < 1e-14);
        ...
        void test_running_stats() {
            ...
            DLIB_TEST_MSG(std::abs(rs.mean() - rsc1.mean_x()) < 1e-10,
                          std::abs(rs.mean() - rsc1.mean_x()));
            ...
            deserialize(rs2, sin);
            // make sure the running_stats and running_scalar_covariance agree
            DLIB_TEST_MSG(std::abs(rs2.mean() - rsc1.mean_x()) < 1e-10,
                          std::abs(rs2.mean() - rsc1.mean_x()));
        ...
        void test_skewness_and_kurtosis_2() {
            ...
            for (int j = 0; j < 5; j++) {
                ...
                unbi_exkur =
                        (n - 1) * ((n + 1) * (exkurnum / exkurdenom - 3) + 6) / ((n - 2) * (n - 3));
        ...
        void another_test() {
            ...
            DLIB_TEST_MSG(std::abs((rc1 + rc2).variance_x() - rc3.variance_x()) < 1e-13,
                          std::abs((rc1 + rc2).variance_x() - rc3.variance_x()));
        ...
        void test_average_precision() {
            ...
            DLIB_TEST(std::abs(average_precision(items) - (2.0 + 4.0 / 5.0 + 4.0 / 5.0) / 4.0) <
                      1e-14);
            DLIB_TEST(std::abs(average_precision(items, 1) - (2.0 + 4.0 / 5.0 + 4.0 / 5.0) / 5.0) <
                      1e-14);
        }
        ...
        void check_distance_metrics(
            ...
            for (unsigned long i = 0; i < samples.size(); ++i) {
                for (unsigned long j = 0; j < samples[i].near_vects.size(); ++j) {
                    const double d1 = length_squared(
                            samples[i].anchor_vect - samples[i].near_vects[j]);
                    for (unsigned long k = 0; k < samples[i].far_vects.size(); ++k) {
                        const double d2 = length_squared(
                                samples[i].anchor_vect - samples[i].far_vects[k]);
                        rs.add(d2 - d1);
                    }
                }
            }
