...
// ----------------------------------------------------------------------------------------
    ...
    template<typename T>
    bool operator==(const future<T> &a, const future<T> &b) { return a.get() == b.get(); }
    ...
    template<typename T>
    bool operator==(const future<T> &a, const T &b) { return a.get() == b; }

    template<typename T>
    bool operator==(const T &a, const future<T> &b) { return a == b.get(); }
...
// ----------------------------------------------------------------------------------------
    ...
    public:
        ...
        uint64 add_task_internal(
            ...
            shared_ptr<function_object_copy> temp;
            return add_task_internal(bfp, temp);
        }
    ...
    private:
        ...
        struct task_state_type {
            task_state_type() : is_being_processed(false), task_id(0), next_task_id(2), arg1(0),
                                arg2(0) { }
        ...
        thread_pool_implementation &operator=(
                thread_pool_implementation &);    // assignment operator
