...
namespace dlib {
    ...
    private:
        ...
        cpp_pretty_printer_kernel_1 &operator=(
                const cpp_pretty_printer_kernel_1 &);    // assignment operator
        ...
        while (type != tok::END_OF_FILE) {
            switch (type) {
                ...
                case tok::KEYWORD: // ---------------------------------------------
                    ...
                    else if (token == "#pragma" || token == "#if" || token == "#define" ||
                             token == "#elif") {
                ...
                case tok::DOUBLE_QUOTED_TEXT: // ----------------------------------
                {
                    if (recently_seen_include) {
                        ...
                        recently_seen_include = false;
                        out << "<a style='text-decoration:none' href='" << htmlify(token) <<
                        ".html'>" << htmlify(token) << "</a>";
                    }
                    else {
