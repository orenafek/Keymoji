...
// -----------------------------------------------------------------------------------------
    ...
    namespace impl {
        ...
        !*/
        {
            ...
            const unsigned long idx_left = fill_parse_tree(parse_tree, item.c.left_tag, back, r,
                                                           k - 1);
            const unsigned long idx_right = fill_parse_tree(parse_tree, item.c.right_tag, back, k,
                                                            c);
                    ...
                    for (itr i = table[k + 1][c].begin(); i != table[k + 1][c].end(); ++i) {
                        for (itr j = table[r][k].begin(); j != table[r][k].end(); ++j) {
                            ...
                            for (unsigned long m = 0; m < possible_tags.size(); ++m) {
                                const double score =
                                        possible_tags[m].second + i->second + j->second;
    ...
    namespace impl {
        ...
        void print_parse_tree_helper(
            ...
            if (tree[i].left < tree.size()) {
                left_recurse = true;
                print_parse_tree_helper<print_tag, skip_tag>(tree, words, tree[i].left, tag_to_skip,
                                                             out);
            }
            else {
            ...
            if (tree[i].right < tree.size()) {
                print_parse_tree_helper<print_tag, skip_tag>(tree, words, tree[i].right,
                                                             tag_to_skip, out);
            }
            else {
...
// -----------------------------------------------------------------------------------------
    ...
    std::string parse_tree_to_string(
        ...
        std::ostringstream sout;
        impl::print_parse_tree_helper<false, false>(tree, words, root_idx, tree[root_idx].tag,
                                                    sout);
        return sout.str();
    }
